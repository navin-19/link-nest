 ...
 * 
 * Fetches and normalizes Google Business Reviews and search results strictly by Place ID.
...
export async function searchGooglePlaces(query) {
  ...
  // Returns realistic matching business data based on the query
  ...
  return [
    ...
    {
      place_id: `${mockId}_branch2`,
      name: `${cleanQuery} (Branch)`,
      address: '45 Second Avenue, Anna Nagar, Chennai, Tamil Nadu',
      rating: 4.8,
      user_ratings_total: 42,
      maps_url: `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(cleanQuery)}&query_place_id=${encodeURIComponent(mockId)}_branch2`,
    },
  ];
}
...
export async function getGoogleReviews(placeId) {
  ...
  // Validate basic Place ID format
