...
// Clean, vector-accurate white official platform logos (20–24px)
function PhoneLogo({ size = 22, className = '' }) {
  return (
...
function TikTokLogo({ size = 22, className = '' }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor" className={className}>
      <path d="M19.59 6.69a4.83 4.83 0 0 1-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 0 1-5.2 1.74 2.89 2.89 0 0 1 2.31-4.64c.298-.002.595.042.88.13V9.4a6.33 6.33 0 0 0-1-.08A6.34 6.34 0 0 0 3 15.66a6.34 6.34 0 0 0 10.86 4.43v-7a8.16 8.16 0 0 0 4.77 1.52v-3.4a4.85 4.85 0 0 1-1.04-.02z" />
    </svg>
  );
}
...
function TwitterXLogo({ size = 22, className = '' }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor" className={className}>
...
function TelegramLogo({ size = 22, className = '' }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor" className={className}>
...
function EmailLogo({ size = 22, className = '' }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}>
...
function WebsiteLogo({ size = 22, className = '' }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}>
...
const PLATFORM_MAP = {
  ...
  tiktok: {
    Icon: TikTokLogo,
    label: 'TikTok',
    format: (v) => (v.startsWith('http') ? v : `https://tiktok.com/${v.startsWith('@') ? v : '@' + v}`),
  },
  twitter: {
    Icon: TwitterXLogo,
    label: 'Twitter / X',
 ...
 * - 44–48px dark rounded-square container (rounded-[14px])
 * - 20–22px white official platform logo
 * - Subtle border, subtle shadow, sleek hover scale
 */
export function SocialIconItem({ item, preview = false, onTrack }) {
  ...
  return (
    <a
      ...
      className="w-11 h-11 sm:w-12 sm:h-12 rounded-[14px] bg-slate-900 hover:bg-slate-850 text-white border border-slate-800/90 shadow-2xs hover:shadow-soft flex items-center justify-center shrink-0 transition-all duration-150 ease-out hover:scale-105 active:scale-95 cursor-pointer select-none"
    >
...
/**
 * SocialIcons Section
 * Renders the centered horizontal row of uniform dark-themed social buttons.
 */
export default function SocialIcons({
  ...
  const items = Object.entries(profileSocial)
    ...
    .map(([platform, rawVal]) => {
      ...
      // 1. Direct configuration from PLATFORM_MAP
      let config = PLATFORM_MAP[platformKey];
      let url = config ? config.format(val) : (val.startsWith('http') ? val : `https://${val}`);

      // 2. If unknown key or generic link, detect platform dynamically from URL value
  ...
  return (
    <nav
      ...
      className="flex items-center justify-center gap-3 sm:gap-3.5 flex-wrap py-2 w-full max-w-md mx-auto"
    >
      {items.map((item) => (
        <SocialIconItem
