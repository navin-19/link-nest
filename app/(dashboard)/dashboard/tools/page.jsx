...
export default function ToolsPage() {
  return (
    <div className="max-w-4xl mx-auto space-y-6 text-slate-900 animate-in fade-in duration-150">
      {/* Page Header */}
      <div>
        <div className="flex items-center gap-2 mb-1">
          <h1 className="text-2xl font-bold text-slate-900 tracking-tight flex items-center gap-2">
            <Wrench size={22} className="text-indigo-600" /> Tools & Utilities
          </h1>
        ...
        {TOOLS.map((tool) => {
          ...
          return (
            <Link
              key={tool.href}
              href={tool.href}
              className="group relative flex flex-col justify-between p-6 rounded-3xl border border-slate-200/90 bg-white hover:border-slate-300 shadow-card hover:shadow-card-hover transition-all duration-200 hover:-translate-y-0.5"
            >
              ...
              {/* Bottom Action link */}
              <div className="pt-6 mt-4 border-t border-slate-100 flex items-center justify-between text-xs font-bold text-slate-700 group-hover:text-indigo-600 transition-colors">
                <span>Open Tool</span>
                <div className="w-7 h-7 rounded-full bg-slate-100 group-hover:bg-indigo-50 flex items-center justify-center transition-colors">
